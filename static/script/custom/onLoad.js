...
Global Variables
-----------------------------------*/
var languages = ["japanese", "english", "romaji"],
    language = "japanese",
    category = "All",
    correct = 0,
    wrong = 0,
...
function resizeText(cardNo, ft, bt1, bt2) {
    // Hiragana does not need to be resized
    if (language != "japanese") {
        ...
        if (bt1.length >= 11) {  // "<=" was ">"
            $("#" + "card" + cardNo + "-bt1").css("font-size", "25pt");
        } else if (bt1.length < 7) { // "<" was ">"
            $("#" + "card" + cardNo + "-bt1").css("font-size", "30pt");
        };
    };

    // Second back text is never Hiragana and always needs to be resized
    if (bt2.length > 11) {
...
function populateCards(array, front) {
    ...
    for (var index = 0; index < (currentIds.length); ++index) {
        ...
        if (language == "japanese") {
            // Add New Values
            $(frontHtmlID).append('<p class="card-text" lang="jp" id="card' + String(index + 1) + '-ft">' + frontText + '</p>');
            ...
            $(backHtmlID).prepend('<p class="card-text" lang="jp" id="card' + String(index + 1) + '-bt1">' +  backText1 + '</p><p class="card-text" lang="en" id="card' + String(index + 1) + '-bt2">' +  backText2 + '</p>');
        };
